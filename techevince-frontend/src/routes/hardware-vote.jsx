...
export default function HardwareVote() {
  ...
  const [votedHardware, setVotedHardware] = useState(false);
  ...
  const loginHandler = () => {
    // set local storage
    localStorage.setItem("redirect", "/vote-hardware");
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        ...
        if (res.data) {
          setSelectedButton(res.data.hardwareVote);
          if (res.data.email) {
            setLoggedBool(true);
          }
          if (res.data.hardwareVote) {
            setVotedHardware(true);
          }
        }
      } catch (err) {
        console.log(err);
